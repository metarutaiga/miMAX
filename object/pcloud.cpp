...
static char const* getCreate(int create)
{
    switch (create) {
    default:
    case 0: return "Box Emitter";
    case 1: return "Sphere Emitter";
    case 2: return "Cylinder Emitter";
    case 3: return "Object-based Emitter";
    }
    return nullptr;
}

static char const* getParticleAxis(int particleAxis)
{
    switch (particleAxis) {
    default:
    case 0: return "Random Direction";
    case 1: return "Direction Vector";
    case 2: return "Reference Object";
    }
    return nullptr;
}
...
static bool particle(Print log, Chunk const& scene, Chunk const& chunk, Chunk const& child, miMAXNode& node)
{
    ...
    int distributionMethod = std::get<int>(paramBlock[0]);
    float speed = std::get<float>(paramBlock[1]);
    float speedVar = std::get<float>(paramBlock[2]);
    int velDir = std::get<int>(paramBlock[3]);
    float x = std::get<float>(paramBlock[4]);
    float y = std::get<float>(paramBlock[5]);
    float z = std::get<float>(paramBlock[6]);
    float var = std::get<float>(paramBlock[7]);
    int birthMethod = std::get<int>(paramBlock[8]);
    int birthRate = std::get<int>(paramBlock[9]);
    int totalNumber = std::get<int>(paramBlock[10]);
    int emitStart = std::get<int>(paramBlock[11]);
    int emitStop = std::get<int>(paramBlock[12]);
    int displayUntil = std::get<int>(paramBlock[13]);
    int life = std::get<int>(paramBlock[14]);
    int lifeVar = std::get<int>(paramBlock[15]);
    int customMtl = std::get<int>(paramBlock[16]);
    float size = std::get<float>(paramBlock[17]);
    float sizeVar = std::get<float>(paramBlock[18]);
    int growTime = std::get<int>(paramBlock[19]);
    int fadeTime = std::get<int>(paramBlock[20]);
    int rndSeed = std::get<int>(paramBlock[21]);
    float emitterWidth = std::get<float>(paramBlock[22]);
    float emitterHeight = std::get<float>(paramBlock[23]);
    float emitterDepth = std::get<float>(paramBlock[24]);
    int emitterHidden = std::get<int>(paramBlock[25]);
    int particleClass = std::get<int>(paramBlock[26]);
    int particleType = std::get<int>(paramBlock[27]);
    float metaTension = std::get<float>(paramBlock[28]);
    float metaTensionVar = std::get<float>(paramBlock[29]);
    float metaCourse = std::get<float>(paramBlock[30]);
    int autoCoarseness = std::get<int>(paramBlock[31]);
    int useSubtree = std::get<int>(paramBlock[32]);
    int animationOffset = std::get<int>(paramBlock[33]);
    int offsetAmount = std::get<int>(paramBlock[34]);
    ...
    int spinTime = std::get<int>(paramBlock[40]);
    float spinTimeVar = std::get<float>(paramBlock[41]);
    float spinPhase = std::get<float>(paramBlock[42]);
    float spinPhaseVar = std::get<float>(paramBlock[43]);
    int spinAxisChoice = std::get<int>(paramBlock[44]);
    float spinAxisX = std::get<float>(paramBlock[45]);
    float spinAxisY = std::get<float>(paramBlock[46]);
    float spinAxisZ = std::get<float>(paramBlock[47]);
    float spinAxisVar = std::get<float>(paramBlock[48]);
    float emitInfluence = std::get<float>(paramBlock[49]);
    float emitMultiplier = std::get<float>(paramBlock[50]);
    float emitMultVar = std::get<float>(paramBlock[51]);
    float bubbleAmp = std::get<float>(paramBlock[52]);
    float bubbleAmpVar = std::get<float>(paramBlock[53]);
    int bubblePeriod = std::get<int>(paramBlock[54]);
    float bubblePeriodVar = std::get<float>(paramBlock[55]);
    float bubblePhase = std::get<float>(paramBlock[56]);
    float bubblePhaseVar = std::get<float>(paramBlock[57]);
    int stretch = std::get<int>(paramBlock[58]);
    int spawnType = std::get<int>(paramBlock[59]);
    int numberOfGens = std::get<int>(paramBlock[60]);
    int numberOfSpawns = std::get<int>(paramBlock[61]);
    float directionChaos = std::get<float>(paramBlock[62]);
    float speedChaos = std::get<float>(paramBlock[63]);
    int speedChaosSign = std::get<int>(paramBlock[64]);
    int inheritOldParticleVelocity = std::get<int>(paramBlock[65]);
    float scaleChaos = std::get<float>(paramBlock[66]);
    int scaleChaosSign = std::get<int>(paramBlock[67]);
    int lifespanEntryField = std::get<int>(paramBlock[68]);
    int constantSpawnSpeed = std::get<int>(paramBlock[69]);
    int constantSpawnScale = std::get<int>(paramBlock[70]);
    float metaCoursenessViewport = 0.0f;
    int spawnPercent = 1;
    float spawnMultVar = 0.1f;
    int notDraft = 0;
    int dieAfter = 10;
    float dieAfterVar = 0.01f;
    int ipcEnable = 0;
    int ipcSteps = 1;
    float ipcBounce = 1.0f;
    float ipcBounceVar = 1.0f;

    if (paramBlock.size() > 71) {
        metaCoursenessViewport = std::get<float>(paramBlock[71]);
    }

    if (paramBlock.size() > 73) {
        spawnPercent = std::get<int>(paramBlock[72]);
        spawnMultVar = std::get<float>(paramBlock[73]);
    }

    if (paramBlock.size() > 74) {
        notDraft = std::get<int>(paramBlock[74]);
    }

    if (paramBlock.size() > 76) {
        dieAfter = std::get<int>(paramBlock[75]);
        dieAfterVar = std::get<float>(paramBlock[76]);
    }

    if (paramBlock.size() > 80) {
        ipcEnable = std::get<int>(paramBlock[77]);
        ipcSteps = std::get<int>(paramBlock[78]);
        ipcBounce = std::get<float>(paramBlock[79]);
        ipcBounceVar = std::get<float>(paramBlock[80]);
    }

    node.text = node.text + format("%-40s : %s", "Particle", "Particle Array") + '\n';
    node.text = node.text + format("%-40s : %s", "Distribution Method", getCreate(distributionMethod)) + '\n';
    node.text = node.text + format("%-40s : %g", "Speed", speed) + '\n';
    node.text = node.text + format("%-40s : %g", "Speed Variation", speedVar) + '\n';
    node.text = node.text + format("%-40s : %s", "Speed Direction", getParticleAxis(velDir)) + '\n';
    node.text = node.text + format("%-40s : %g", "X", x) + '\n';
    node.text = node.text + format("%-40s : %g", "Y", y) + '\n';
    node.text = node.text + format("%-40s : %g", "Z", z) + '\n';
    node.text = node.text + format("%-40s : %g", "Speed Variation", var) + '\n';
    ...
    node.text = node.text + format("%-40s : %g", "Emitter Height", emitterHeight) + '\n';
    node.text = node.text + format("%-40s : %g", "Emitter Depth", emitterDepth) + '\n';
    ...
    node.text = node.text + format("%-40s : %g", "Metaparticle Coarseness Viewport", metaCoursenessViewport) + '\n';
    ...
    node.text = node.text + format("%-40s : %s", "One Connected Blob", getBoolean(notDraft)) + '\n';
    node.text = node.text + format("%-40s : %d", "Die X frames after collision", dieAfter) + '\n';
    node.text = node.text + format("%-40s : %g", "Die X frames after collision variation", dieAfterVar) + '\n';
...
static bool register_object = miMAXNode::RegisterObject(PCLOUD_CLASS_ID, particle);
